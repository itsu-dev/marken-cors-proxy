...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // customize
    if (location.host === 'github.com' && location.pathname === '/login/oauth/access_token') {
      const additional = `&client_id=${process.env.CLIENT_ID}&client_secret=${process.env.CLIENT_SECRET}`;
      location.search += additional;
      location.query += additional;
      location.path += additional;
      location.href += additional;
    }
